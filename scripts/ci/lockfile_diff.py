...
def render_markdown(diffs: dict[str, dict[str, list]]) -> str:
    ...
    header = (
        ...
        f"{'s' if total != 1 else ''})\n\n"
        "This PR changes locked npm dependency versions. Review the actual "
        "version movements below — the raw lockfile diff is noise "
        "(reordering + integrity hashes), this table is the signal.\n"
    )
    return header + "\n" + "\n\n".join(sections) + "\n"
